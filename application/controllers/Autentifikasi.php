...
class Autentifikasi extends CI_Controller
{
	...
	private function _sendEmail($token, $type)
	{
		$config = [
			...
			'smtp_user' => 'academyelc2@gmail.com',
			'smtp_pass' => 'UBSI2021',
			'smtp_port' => 465,
			'mailtype'  => 'html',
			'charset'   => 'utf-8',
		...
		$this->email->from('academyelc2@gmail.com', 'ELC Academy');
		$this->email->to($this->input->post('email'));

		if ($type == 'verify') {
			$this->email->subject('Account Verification');
			$this->email->message('Click this link to verify youre account : <a href="' . base_url() . 'autentifikasi/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Activate</a>');
		} else if ($type == 'forgot') {
			$this->email->subject('Reset Password');
			$this->email->message('Click this link to reset youre password : <a href="' . base_url() . 'autentifikasi/resetpassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Password</a>');
		}
